...
%>
		...
		mbc: <%=mbc%><br/>
		<hr/>
		<p>EL을 사용한 값을 전송 받을 때는 $ {param.xxx} 사용 </p>
		아이디 : ${param.mid}<br/>
		비밀번호 : ${param.pwd}<br/>
		성명 : ${param.name}<br/>
		mbc: ${param.mbc}<br/>
...
%>
		...
		atom4[0] > atom4[1] 중 더 큰 값 : ${atom4[0] > atom4[1] ? atom4[0] : atom4[1]}<br/><br/>
		atom4[0] gt atom4[1] : ${atom4[0] gt atom4[1]}<br/><br/>
		atom4[0] gt atom4[1] 중 더 큰 값 : ${atom4[0] gt atom4[1] ? atom4[0] : atom4[1]}<br/><br/>
		...
		<!-- JSTL을 사용하여 배열 값을 변수에 저장 후 출력해야 한다. 
		hobby : $ {param.hobby[1]}<br/>
		hobby : $ {param.hobby[2]}<br/>
		 -->
		 <hr/>
		 <h3>객체를 전달받기</h3>
<%
...
%>
	...
	연산자(== 과 null을 동시에 비교는 EL에서는 empty(너무 좋은 연산자)) : ${empty var1}<br/>
	연산자(!= 과 not null을 동시에 비교는 EL에서는 !empty(너무 좋은 연산자)) : ${!empty var1}<br/>
	연산자(!= 과 not null을 동시에 비교는 EL에서는 not empty(너무 좋은 연산자)) : ${not empty var1}<br/>
