	...
	<form name="adminForm">
		<table class="table table-borderless m-0">
			...
			<tr>
				...
				<td style="text-align: right;"> 회원 등급
					<select name="level" onchange="levelSearch()">
						<option value="" <c:if test="${level==99}">selected</c:if> >전체회원</option>
...
<div style="text-align:center">
  <c:if test="${pag != 1}">[<a href="${ctp}/adMemberList.ad?pag=1&level=${level}" class="btn btn-secondary btn-sm">첫페이지</a>]</c:if>
  <c:if test="${pag > 1}">[<a href="${ctp}/adMemberList.ad?pag=${pag-1}&level=${level}" class="btn btn-secondary btn-sm">이전페이지</a>]</c:if>
  ${pag}Page / ${totPage}pages
  <c:if test="${pag < totPage}">[<a href="${ctp}/adMemberList.ad?pag=${pag+1}&level=${level}" class="btn btn-secondary btn-sm">다음페이지</a>]</c:if>
  <c:if test="${pag != totPage}">[<a href="${ctp}/adMemberList.ad?pag=${totPage}&level=${level}" class="btn btn-secondary btn-sm">마지막페이지</a>]</c:if>
</div>
<!-- 페이징처리 끝 -->
