...
select name as '이름', gender as '성별', jikwi '직위', address '주소' from insarok;
...
-- 남자자료 처음부터 4건만 출력하시오?
select * from insarok where gender = '남자' limit 0, 4;


-- 남자자료 처음부터 4건을 나이 내림차순으로 출력하시오?
select * from insarok where gender = '남자' order by age desc limit 0, 4 ;
...
update insarok set netpay = netpay + 50000, jikwi = '대리' where jikwi = '사원' and buser = '영업과';
...
select gender, address from insarok where gender = '여자';
